...
class NonBallotInfoTableViewCell: BaseTableViewCell, BindableType {
    ...
    func bindViewModel() {
        ...
        self.viewModel.textValue.asDriver().drive(self.infoLabel.rx.text).disposed(by: self.disposeBag)
        self.bindStyle()
    }
    
    private func bindStyle() {
        
        self.viewModel.textSize.asDriver()
            .drive(onNext: { [weak self] size in
                self?.infoLabel?.font = UIFont.systemFont(ofSize: CGFloat(size))
            }).disposed(by: self.disposeBag)
        
        self.viewModel.isCentered.asDriver()
            .drive(onNext: { isCentered in
                self.infoLabel?.textAlignment = isCentered ? .center : .left
            }).disposed(by: self.disposeBag)
        
        self.viewModel.isBold.asDriver()
            .drive(onNext: { [weak self]  isBold in
